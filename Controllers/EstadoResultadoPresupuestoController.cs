...
{
    public class EstadoResultadoPresupuestoController : Controller
    {
    ...
    // Programa
        ...
        public ActionResult reporteRegionEstandar(int RegionID, int Mes, int? periodo, string TipoReporte)
        {
            ...
            ProPresupuesto = db.Presupuesto.Where(m => m.Activo != null && m.Activo.Equals("S") && m.Periodo == periodo && m.Proyecto.Direccion.Comuna.RegionID == RegionID ).OrderByDescending(p => p.ID).ToList();
            ...
            ViewBag.DetallePresupuesto = detallePresupuesto.ToList();
    ...
    // Linea 
        ...
        public ActionResult ReporteLineaEstandar(int LineaID, int Mes, int? periodo, string TipoReporte)
        {
            ...
            ProPresupuesto = db.Presupuesto.Where(m => m.Activo != null && m.Activo.Equals("S") && m.Periodo == periodo && m.Proyecto.TipoProyecto.LineasAtencionID == LineaID && m.Proyecto.LF == 0).OrderByDescending(p => p.ID).ToList();
            foreach (var Registo in ProPresupuesto)
            {
                int prID = Registo.ProyectoID;
                var xRevisar = Proyectos.Where(d => d.ProyectoID == prID).FirstOrDefault();
    ...
    // Tipo Programa
        ...
        public ActionResult reporteTipoEstandar(int TipoProyectoID, int Mes, int? periodo, string TipoReporte)
        {
            ...
            ProPresupuesto = db.Presupuesto.Where(m => m.Activo != null && m.Activo.Equals("S") && m.Periodo == periodo && m.Proyecto.TipoProyectoID == TipoProyectoID).OrderByDescending(p => p.ID).ToList();
            foreach (var Registo in ProPresupuesto)
            {
                int prID = Registo.ProyectoID;
                var xRevisar = Proyectos.Where(d => d.ProyectoID == prID).FirstOrDefault();
